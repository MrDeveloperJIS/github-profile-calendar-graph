...
const INITIAL_DELAY_SEC = 1; // brief hold on the starting frame before the sweep begins
 ...
 * @param {number} [opts.borderRadius] - card corner radius in px, default 0
 ...
 * @param {{text?: string, background?: string, border?: string}} [opts.colorOverrides] -
 *   pre-sanitized '#rrggbb' values from `?color=`/`?bg-color=`/`?border=`. Only overrides
 *   card chrome (labels/background/border) — the contribution-square colors stay fixed
 *   to the theme's scale regardless.
 */
function buildCalendarSvg({
    ...
    const t = {
        ...baseTheme,
        text: colorOverrides.text || baseTheme.text,
        background: colorOverrides.background || baseTheme.background,
        border: colorOverrides.border || baseTheme.border,
    };
  ...
  <rect x="${inset}" y="${inset}" width="${width - bw}" height="${contentHeight - bw}" rx="${borderRadius}" fill="${t.background}"${borderAttrs} />
